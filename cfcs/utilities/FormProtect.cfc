...
<cffunction name="Test" access="public" returntype="boolean" output="no" hint="Master test method - assigns the various tests to the fields, and returns a boolean true if ok to proceed or false if the submission fails.">
	...
	<cfelse>
		<!----[   do other tests  ]---->
		<cfif testmessage( formfield.message ) eq "NOTOK"><cfset SubmissionOK = false /></cfif>
	</cfif>
...
<cffunction name="testmessage" access="public" output="no" returntype="string" hint="tests a string against a series of tests to check for spam content">
<cfargument name="argsMessage" type="string" required="yes" />
<cfset var message = arguments.argsMessage />
<cfset var result = "Ok" />

	<cfif (message contains "<a href=") AND ( message contains "[url=" ) AND (   message contains "[link=" )>
		<cfset result = "NOTOK" />
	</cfif>	  
<cfreturn result />
</cffunction>
