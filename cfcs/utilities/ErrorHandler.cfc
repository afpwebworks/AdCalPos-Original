...
Revision history: 
4/2010 modified for extensive use throughout the application for general error handling and reporting.
...
<cffunction name="MakeErrorString" returntype="string" access="public" output="false" hint="Takes the error struct and converts it to a comma delimited string">
<cfargument name="eH" type="ErrorHandler" required="yes" />
<cfset var errorstruct = arguments.eh.geterrors() />
<cfset var errorstring = "" />
<cfset var CorrectedErrorString = "" />

<cfloop list="#arraytolist(StructKeyArray(errorstruct))#" index="i"><cfoutput>
<cfset thiserrorarray = #evaluate("errorstruct." & i)# />
<!---[   <cfset errorstring = errorstring & "<li>" />   ]----><!----[   & #i# & ": " />   ]---->
<cfif arraylen(thiserrorarray) eq "1">
	<cfset errorstring = errorstring & #i# & ": "& #thiserrorarray[1]#  />
<cfelse>
	<cfset errorstring = errorstring & "Errors:" />
	<cfloop list="#arraytolist(thiserrorarray)#" index="j"><cfoutput>
		<cfset errorstring = errorstring &  #i# & ": " & #j# & ", " />
	</cfoutput></cfloop>
	<cfset errorstring = errorstring & "," />
</cfif>
</cfoutput></cfloop>
<cfset errorstring = errorstring & "." />
<!---[   Tidy up the punctuation on the string   ]---->
<cfset CorrectedErrorString = replace(errorstring, ",,", ",", "ALL") />
<cfset CorrectedErrorString = replace(CorrectedErrorString, ".,", ".", "ALL") />
<cfset CorrectedErrorString = replace(CorrectedErrorString, ",.", ".", "ALL") />
<cfreturn CorrectedErrorString />
</cffunction>
