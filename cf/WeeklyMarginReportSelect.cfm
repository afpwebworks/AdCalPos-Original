...
<!--- - wb 12/11/2003 - Setup page title - --->
...
<table width="100%">
  <tr valign="top"> 
    <td width="25%"><a href="MainMenu.cfm" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('home','','../images/butHomeDown.gif',1)" target="_top"><img name="home" border="0" src="../images/butHomeUp.gif" width="114" height="34" align="top"></a></td>
 	<td align="center"><h1><cfoutput>#local.pageTitle#</cfoutput></h1><br /><h2>Please select a day.</h2></td>
    <td width="25%"> 
...
<table align="center" border="1" bordercolor="#4B4B4B" cellpadding="2" cellspacing="0">
	...
	<tr><!--- - create counters for the calendar - --->
		...
		<cfloop from="1" to="42" index="i">
			...
			<cfif local.dateCount LTE daysInMonth(local.cDate) AND dayOfWeek(createDate(year(local.cDate),month(local.cDate),local.dateCount)) EQ local.cCount>
				...
				<td align="center" class="normal_body"><cfif i/local.count EQ 7><a<cfif year(local.cDate) EQ year(now()) AND month(local.cDate) EQ month(now()) AND local.dateCount EQ day(now())> class="today"</cfif> href="WeeklyMarginReport.cfm?date=<cfoutput>#year(local.cDate)##numberFormat(month(local.cDate),00)##numberFormat(local.dateCount,00)#</cfoutput>&amp;RequestTimeout=300"><cfelse><span<cfif year(local.cDate) EQ year(now()) AND month(local.cDate) EQ month(now()) AND local.dateCount EQ day(now())> class="today"<cfelse> class="cal_disabled"</cfif>></cfif><cfoutput>#numberFormat(local.dateCount,99)#</cfoutput><cfif i/local.count EQ 7></a><cfelse></span></cfif></td>
	...
	<tr>
		<td class="cal_days"><a href="WeeklyMarginReportSelect.cfm?pos=bwd"><img src="../images/s.gif" width="5" height="1" alt="spacer" border="0" />&lt;<img src="../images/s.gif" width="5" height="1" alt="spacer" border="0" /></a></td>
		<td class="cal_days" colspan="5"><img src="../images/s.gif" width="1" height="1" alt="spacer" border="0" /></td>
		<td class="cal_days"><a href="WeeklyMarginReportSelect.cfm?pos=fwd"><img src="../images/s.gif" width="5" height="1" alt="spacer" border="0" />&gt;<img src="../images/s.gif" width="5" height="1" alt="spacer" border="0" /></a></td>
	</tr>
</table>
