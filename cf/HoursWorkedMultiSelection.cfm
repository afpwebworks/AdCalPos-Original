...
<br>
<br>
	<!--- temporary option for hiostorical saturdays --->
	<br>
	<table width="100%" border="0">
	  <tr>
	    <td>
	      <div align="center">
		   <CFOUTPUT>
			<FORM action="HoursWorkedMultiple.cfm" method="post">
				<select name="weekEndingDate">
				    <CFSET nextSat = 7 - dayOfWeek(now())>
					<cfloop index ="loop" from="#nextSat#" to="-500" step="-1">
					    <CFSET thisdate = #dateAdd('d',loop,now())#>
						<CFSET thisDOW = #dayOfWeek(thisDate)#>
						<CFIF thisDow is 7>
							<option value='#dateFormat(thisdate,"YYYY-MM-DD")#'>#dateFormat(thisdate,"dd/mm/yyyy")#</option>
						</cfif>	
					</cfloop>
				</select><p></P>
