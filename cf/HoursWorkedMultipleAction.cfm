...
<CFTRANSACTION>
	<CFOUTPUT>
	  <CFSET loop=1>
	  <cfloop condition='ISdEFINED("RosterID_Line#loop#")'>
	    <cfif evaluate("Time_To_Line#loop#") lt evaluate("Time_From_Line#loop#")>
		  Line #loop# has a start time later than the end time, please amend
		  <CFABORT>
		</cfif>
	    <cfif evaluate("Entitlement_Time_To_Line#loop#") lt evaluate("Entitlement_Time_From_Line#loop#")>
		  Line #loop# has an entitlement start time later than the end time, please amend
		  <CFABORT>
		</cfif>	  
		<cfif ( evaluate("Entitlement_Time_To_Line#loop#") is not 0 or evaluate("Entitlement_Time_From_Line#loop#") is not 0) 
		 and #evaluate("EntType_Line#loop#")# is 'NA'>
		   Please choose the entitlement type for line #loop#
		  <CFABORT>
		</cfif>
  ...
  <cflocation url="HoursWorkedMultiple.cfm?weekEndingDate=#weekEndingDate#">
<cfabort>
