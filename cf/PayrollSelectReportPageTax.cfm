...
<cfset strPageTitle = "Tax Summary">
...
<cfset strQuery = "SELECT Sum(tblEmpPayRollPaid.Tax) AS Tax ">
<cfset strQuery = strQuery & "FROM tblEmpPayRollPaid ">
<cfset strQuery = strQuery & "WHERE (((tblEmpPayRollPaid.StoreID)=#lngSID#) AND ((10000*substring([WeekEnding],5,4)+100*substring([WeekEnding],3,2)+substring([WeekEnding],1,2)) Between #strDFSort# And #strDTSort#) AND ((tblEmpPayRollPaid.TaxPaidDate)='' Or (tblEmpPayRollPaid.TaxPaidDate) Is Null))">
...
<cfif #GetData.RecordCount# GT 0>
	<FORM action="PayrollSelectReportPageTaxAction.cfm" method="post">
	<cfoutput>
		...
		<INPUT TYPE="hidden" NAME="dblTax" VALUE="#numberformat(GetTotalData.Tax,"_____.00")#">		
	</cfoutput>
