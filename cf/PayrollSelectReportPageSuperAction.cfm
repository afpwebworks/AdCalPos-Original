...
<cfset strPageTitle = "Super Payment">
...
<cfset dblSuper = "#Form.dblSuper#">

	<!--- Get UUID to have a unique identifier --->
	<cfset UUID = CreateUUID()>
	
	<!--- add a record to the super paid directory --->
	<cfset strQuery = "INSERT INTO tblEmpSuperPaid ( StoreID, SuperAmount, ReferenceNumber, StartDate, EndDate, RecordUUID ) ">
	<cfset strQuery = strQuery & "Values( #lngSID#, #dblSuper#, '#strRefNumber#', '#strDF#', '#strDT#', '#UUID#') ">
	<CFQUERY name="AddTaxPaid" datasource="#application.dsn#" > 
	<!--- <CFQUERY name="AddTaxPaid" DBTYPE="OLEDB" PROVIDER="#AppProvider#" dataSource="#AppCostiDB1#" PROVIDERDSN="#AppCostiDB1#" USERNAME="admin"> --->
		#PreserveSingleQuotes(strQuery)#
	</CFQUERY>

	<!--- Get the record ID --->
	<cfset strQuery = "select SuperPaidID from tblEmpSuperPaid Where RecordUUID = '#UUID#' ">
	<CFQUERY name="GetID" datasource="#application.dsn#" > 
		#PreserveSingleQuotes(strQuery)#
	</CFQUERY>
	<cfset NewID = 0>
	<cfif #GetID.RecordCount# GT 0>
		<cfif #isnumeric(GetID.SuperPaidID)#>
			<cfset NewID = #GetID.SuperPaidID# >
		</cfif>
	</cfif>
	
	<!--- Mark all of the related records --->
	<cfset strQuery = "UPDATE tblEmpPayRollPaid SET tblEmpPayRollPaid.SuperPaidID = #NewID#, tblEmpPayRollPaid.SuperPaid = 1, tblEmpPayRollPaid.SuperChequeNo = '#strRefNumber#', tblEmpPayRollPaid.SuperPaidDate = replace(str(datepart(dd, getdate()),2),' ','0') + replace(str(datepart(mm, getdate()),2),' ','0') + str(datepart(yyyy, getdate()),4) ">
    ...
    <td>
      <div align="center">
        <table width="80%" border="0">
			<tr>
				<td>
					<cfoutput>Successfully saved the super payment.</cfoutput>
				</td>
			</tr>
        </table>
	  </div>
