...
<cfset local.formName="frmStockOnHandByDAte">
<cfset local.page="StockOnHandDateSelection.cfm">

<cfset strPageTitle = "Stock On Hand By Date">
...
<FORM action="StockOnHandByDate.cfm" id="<cfoutput>#local.formName#</cfoutput>" method="post" name="<cfoutput>#local.formName#</cfoutput>" onsubmit="return submitCheck('<cfoutput>#local.formName#</cfoutput>');">
...
<br>
<br>
<cfoutput><input type="hidden" name="lngStoreID" value="#session.storeid#"></cfoutput>
      ...
      <div align="center">
        <p><h2>Please select the department</h2>
			...
			<select name="cmbDeptNo">
    	      ...
    	      &nbsp; 
        <p>
          <input type="submit" name="Submit" value="Stock on Hand" class="buttonWidth">
        </p>
		
		<p><input id="btnSubmit" name="btnSubmit" onclick="this.form.action='StockAdjustmentsReport.cfm'" type="button" value="Stock Adjustments Report"  class="buttonWidth">
		</p>
