...
<cfset strPageTitle = "Payment Application (1 / 3)">
...
<cfset strQuery = strQuery & "FROM tblStores INNER JOIN tblPayment ON tblStores.StoreID = tblPayment.StoreID ">
<cfset strQuery = strQuery & "WHERE (((tblPayment.FullyApplied)=0)) ">
<cfset strQuery = strQuery & "GROUP BY tblStores.StoreID, tblStores.StoreName, tblStores.NoLongerUsed ">
<cfset strQuery = strQuery & "HAVING (((tblStores.StoreID)<>1) AND ((tblStores.NoLongerUsed)=0)) ">
<cfset strQuery = strQuery & "ORDER BY tblStores.StoreName">
 --->
<cfset strQuery = "SELECT tblStores.StoreID, tblStores.StoreName ">
<cfset strQuery = strQuery & "FROM tblStores INNER JOIN tblPayment ON tblStores.StoreID = tblPayment.StoreID ">
<cfset strQuery = strQuery & "WHERE (((tblStores.NoLongerUsed)=0) AND ((tblPayment.FullyApplied)=0)) ">
<cfset strQuery = strQuery & "GROUP BY tblStores.StoreID, tblStores.StoreName ">
<cfset strQuery = strQuery & "HAVING (((tblStores.StoreID)<>1)) ">
<cfset strQuery = strQuery & "ORDER BY tblStores.StoreName">
...
<CFQUERY name="GetStores"      datasource="#application.dsn#" > <!--- <CFQUERY name="GetStores"      dataSource="#AppCostiDB1#"       USERNAME="admin"> --->
	...
	<cfset strQuery = "SELECT tblStores.StoreID, tblStores.StoreName ">
	<cfset strQuery = strQuery & "FROM tblStores INNER JOIN tblPayment ON tblStores.StoreID = tblPayment.StoreID ">
	<cfset strQuery = strQuery & "WHERE (((tblStores.NoLongerUsed)=0) AND ((tblPayment.FullyApplied)=0))  and tblStores.StoreID = #Session.StoreID# ">
	<cfset strQuery = strQuery & "GROUP BY tblStores.StoreID, tblStores.StoreName ">
	<cfset strQuery = strQuery & "HAVING (((tblStores.StoreID)<>1)) ">
	<cfset strQuery = strQuery & "ORDER BY tblStores.StoreName">
...
<br>
<br>
<FORM action="Payment_AppB.cfm" method="post">
		...
		<TD>
			...
			<cfelseif #lngUserType# GE 6 and #lngUserType# LE 9> 
				...
				<cfoutput Query = "GetCurrentStore">
					<!--- <cfif #GetCurrentStore.RecordCount# GE 1> --->
						<option value="#Session.StoreID#" selected>#GetCurrentStore.StoreName#</option>	<!--- 		</cfif> --->
					</cfoutput>
				</select>
				<input type="hidden" name="StoreID_integer" value="Please select store">
				<input type="hidden" name="StoreID_required" value="Please select store">
