...
<cfset local.formName="frmPackingReportRequest">
<cfset local.page="PackingReportRequest.cfm">
...
<cfelse>
	<cfset strQuery = "SELECT OrderDate ">
	<cfset strQuery = strQuery & "FROM qryOrderDatesBbyStore ">
...
<cfif #GetOrderDates.RecordCount# LT 1>
	<BR><p><h2>There are no orders to be packed.</h2>
	<cfabort>
</cfif>
<cfset local.lsDates="">
...
<p><h3>Please type the order date.</h3>
<FORM action="PackingReportActionA.cfm" id="<cfoutput>#local.formName#</cfoutput>" method="post" name="<cfoutput>#local.formName#</cfoutput>" onsubmit="return submitCheck('<cfoutput>#local.formName#</cfoutput>');">
